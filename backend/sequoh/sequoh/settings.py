...
if not SECRET_KEY:
    if os.environ.get('ENVIRONMENT', 'development') == 'production' or os.environ.get('RENDER') or os.environ.get('DATABASE_URL'):
        raise ImproperlyConfigured('SECRET_KEY must be set in the environment for production deployments.')
...
REST_FRAMEWORK = {
    ...
    # Rate limiting: solo se limita a las vistas que definen `throttle_scope`.
    # Las vistas sin scope se comportan como antes (sin limitación).
...
# JWT (PyJWT) - configuración de emisión/validación
# Clave DEDICADA de firma de tokens, separada del SECRET_KEY maestro del framework.
# Permite rotar tokens sin invalidar sesiones Django y viceversa.
JWT_SECRET_KEY = os.getenv('JWT_SECRET_KEY', SECRET_KEY)
JWT_ALGORITHM = os.getenv('JWT_ALGORITHM', 'HS256')
JWT_ISSUER = os.getenv('JWT_ISSUER', 'genomia')
JWT_AUDIENCE = os.getenv('JWT_AUDIENCE', 'genomia-app')
JWT_EXPIRATION_HOURS = int(os.getenv('JWT_EXPIRATION_HOURS', '8'))

# Cookie de autenticación (JWT en HttpOnly).
# En prod (cross-site, https): SameSite=None; Secure=True.
# En dev (proxy Vite, same-origin): SameSite=Lax; Secure=False.
...
AUTH_COOKIE_SECURE = os.getenv('AUTH_COOKIE_SECURE', '0' if DEBUG else '1') not in ('0', 'False', 'false')
...
# Adapter que envía con Gmail API (usa accounts/adapters.py)
ACCOUNT_ADAPTER = 'accounts.adapters.GmailAPIAccountAdapter'
