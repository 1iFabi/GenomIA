...
export function useSunburstBreadcrumbItems() {
  ...
  const items = useMemo(() => {
    ...
    while (cur) {
      crumbs.unshift(cur);
      cur = cur.parentId ? focusById.get(cur.parentId) : void 0;
    }
...
export const SunburstBreadcrumb = memo(function SunburstBreadcrumb2({
  className,
  children
}) {
  return <nav aria-label="Drill-down path" className={className ?? "mb-4"}>
      {children}
    </nav>;
});
