...
function layoutNode(node, id, depth, a0, a1, parentId, categoryIndex, trail, ctx) {
  ...
  if (depth > 0) {
    ctx.arcs.push({
      ...
      riskKey: node.riskKey,
      chartNodeType: node.chartNodeType,
      a0,
      a1,
...
export function ringOptions(focusDepth, maxDepth, radius) {
...
export function buildSunburstEnterTiming(arcs, staggerScale = 1) {
  ...
  const segmentDelays = /* @__PURE__ */ new Map();
  let maxDelay = 0;
...
export function buildRevealSchedule(arcs, staggerScale = 1) {
  ...
  const ringStarts = /* @__PURE__ */ new Map();
  for (const arc of arcs) {
...
export function arcPath(geometry, progress, radialProgress = progress) {
  ...
  const currentOuter = innerR < 1 ? outerR * radialP : innerR + (outerR - innerR) * radialP;
...
function arcPathFromRadii(currentA0, currentA1, currentInner, currentOuter) {
  if (currentOuter - currentInner < 0.5 || currentA1 - currentA0 < 1e-3) {
    return null;
  }
...
export function buildHoverGrowTargets(arcs, hoveredArc, focus, maxDepth, radius, hoverPop, isOnHoverPath) {
  ...
  for (const d of arcs) {
    ...
    const baseThickness = base ? base.outerR - base.innerR : maxExpandedThickness;
    const allowedGrow = baseThickness >= maxExpandedThickness ? 0 : Math.min(segmentGrow, maxExpandedThickness - baseThickness);
